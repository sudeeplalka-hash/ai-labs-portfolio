...
export async function extractTextFromFile(file: File): Promise<string> {
  ...
  try {
    if (ext === "pdf") {
      ...
      if (!looksLikeText(text)) {
        throw new FileExtractionError(
          `"${file.name}" has no selectable text (it may be a scanned image). Try a text-based PDF or .docx.`,
        );
      }
      return text;
    }
    if (ext === "docx") {
      ...
      if (!looksLikeText(text)) {
        throw new FileExtractionError(`"${file.name}" didn't contain readable text.`);
      }
      return text;
    }
    if (ext === "doc") {
    ...
    if (!looksLikeText(text)) {
      throw new FileExtractionError(`"${file.name}" doesn't look like readable text.`);
    }
    return text;
  } catch (err) {
    if (err instanceof FileExtractionError) throw err;
    throw new FileExtractionError(`Couldn't read "${file.name}". Try .txt, .md, .csv, .pdf, or .docx.`);
  }
}

export const CORPUS_UPLOAD_ACCEPT =
  ".csv,.json,.txt,.md,.markdown,.tsv,.pdf,.docx,text/plain,text/markdown,application/pdf,application/vnd.openxmlformats-officedocument.wordprocessingml.document";
