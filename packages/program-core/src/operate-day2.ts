// ============================================================================
// Operate (stage 07), day two continuous operations engine.
...
// day two forever: drift over time, canary decay, staleness, agent behavior,
...
export function valueAtRisk(s: ProgramState, series: OpsSeries): ValueAtRisk {
  // Defensive reads: Realize's model when present (riskAdjustedValue $/yr, adoption 0 to 1), honest defaults otherwise.
...
// ---- The authored day two incident arc ------------------------------------------
...
export function buildWeeklyOpsReview(s: ProgramState, series: OpsSeries, signals: OpsSignal[]): string {
  ...
  const lines = [
    ...
    ``,
    `## Decision pending`,
    signals.length ? `Remediation call required: retrain / reindex / rollback / rescope (see incident INC-OP-007).` : `None this week.`,
    FOOTER(s),
  ];
  return lines.join("\n");
}
...
// Forward projection, the day two point is to act before the breach, not after. Fit the recent
