...
// `firsthand` badge is only representable WITH owner sign off, so a record that
...
// Owner-confirmed firsthand domains (signed off 2026-07-03). Each must appear ≥3×.
...
// Provenance is a discriminated union: `firsthand` is ONLY representable with an
// explicit owner sign off, so an unsigned firsthand claim is a compile error.
export type Provenance =
  | { kind: "firsthand"; ownerSignedOff: true }
  | { kind: "studied" };
...
export function assertUseCases<P>(list: UseCase<P>[]): UseCase<P>[] {
  for (const uc of list) {
    ...
    if (uc.provenance.kind === "firsthand" && !uc.provenance.ownerSignedOff)
      throw new Error(`[use cases] ${uc.id}: firsthand requires owner sign off.`);
  }
  return list;
}
...
export function coverageFrom(list: UseCase[]): {
  ...
  for (const uc of list) {
    ...
    cur.total += 1;
    if (uc.provenance.kind === "firsthand") cur.firstHand += 1;
