...
// Stage 07 · Operate, day two continuous operations (SPEC-OPERATE-STAGE-V2).
// Placed after Realize on the rail, scoped from Deploy onward, and defined by the
// loop-back: drift/staleness evidence becomes a retrain / reindex / rollback /
// rescope decision that feeds Frame, Build, Deploy, Realize, and Govern.
// Three views, one authored incident arc, capped scope. SIMULATED throughout, // seeded deterministic series, never telemetry.
//
...
const LOOP_LABEL: Record<string, string> = { frame: "→ Frame (rescope)", build: "→ Build (retrain/reindex)", deploy: "→ Deploy (restrict)" };
...
const KPI_TIP = {
  slo: "Service Level Objectives: the classic “is it up and fast?” health, covering availability, 95th percentile latency (p95), and error rate. In this incident they stay green the whole time. That is the trap: infra health tells you nothing about whether the answers are still correct.",
  canary: "A fixed set of known good questions (a “canary” set inherited from Build's golden dataset) rerun on a schedule. The share that still return correct answers. Unlike SLOs it tracks answer quality, so it is what catches silent decay. A drop of 8+ points below the Build baseline while SLOs stay green is the “silent drift” signal.",
  staleness: "Days since the retrieval index / knowledge base was last successfully refreshed against the source of truth. As it grows, the model answers from an outdated corpus and recall plus citation accuracy fall. Past the tier threshold (21 days, or 14 for high governance) it is a breach.",
  cost: "Fully loaded inference cost for one completed task. It creeps about 2% per week from prompt growth, a lower cache hit rate raises it, and agent “loop anomalies” (retries / repeated actions) add wasted calls. Cheap to ignore, expensive at scale.",
  valueAtRisk: "Annualized dollars of realized value exposed while quality is degraded, equal to annual value times adoption times quality degradation percent. It converts answer decay into money so Realize can net it out of the business case.",
} as const;

// Secondary terms, kept in one place so the cards stay uncluttered.
const GLOSSARY: { term: string; def: string }[] = [
  ...
  { term: "Loop anomalies", def: "Agent runs whose action fingerprint looks off, with repeated steps or excess iterations. A behavior drift signal that also wastes cost." },
  { term: "Governance tier", def: "The risk tier assigned back in Govern. Higher tiers tighten thresholds, for example a staleness breach fires at 14 days instead of 21." },
  { term: "Value at risk", def: "Annualized value exposed while quality is degraded, equal to annual value times adoption times quality degradation percent. The bridge from answer decay to dollars, for Realize." },
  { term: "Blast radius", def: "Who and what an incident affects. Here, that means answer quality on recent document questions, citation accuracy, and (with a lag) user trust and adoption." },
];

const REMEDIATION_TIP =
  "Each option fixes a different layer and loops back to a different stage. Reindex (→ Build): fix the refresh pipeline and rebuild the index, a cheap root cause fix. Retrain (→ Build): refresh the model and rebaseline the golden set, thorough but slow. Rollback (→ Deploy): restrict to covered documents, an honest degraded mode that buys time but fixes nothing. Rescope (→ Frame): reframe the initiative around the document types that carry the value. “Loops back to” is the upstream stage Operate sends the work to, the point of the stage: a loop, not a ticket.";
...
export function OperateStage() {
  ...
  const yf = (v: number) => padT + ((100 - v) / 45) * plotH; // domain 55 to 100
  ...
  const STEPS: { k: View; n: string; label: string; hint: string; Icon: typeof Activity }[] = [
    ...
    { k: "incident", n: "2", label: "Decide", hint: "Pick a fix: reindex, retrain, rollback, or rescope.", Icon: AlertTriangle },
    ...
    <div>
        ...
        <div className="space-y-4">
          ...
          <Panel>
            ...
            <p className="mt-2 text-[11px] text-slatey-500">
              ...
              This is EL-04&apos;s reported versus actual lesson at model altitude.
            </p>
          </Panel>
        ...
        <div className="space-y-4">
          <Panel>
            ...
            <p className="mt-2 text-[11px] text-slatey-500">
              ...
              <MetricTooltip className="ml-1 align-middle" text="Blast radius equals who and what the incident touches. Here: answer quality on recent document questions, citation accuracy, and, with a lag, user trust and adoption." />
            </p>
          </Panel>
          ...
          {feedback && (
            <InsightCard title={`Decision recorded: ${feedback.decision.label}`} tone="success">
              Loopback issued {LOOP_LABEL[feedback.decision.loopTarget]}. See <button onClick={() => setView("loop")} className="font-semibold text-primary hover:underline">the loop</button>.
      ...
      <div className="mt-8 space-y-4 border-t border-line pt-6">
        ...
        <p className="text-xs text-slatey-500">
          ...
          Real day two operations need telemetry ingestion, alert routing, and on call staffing. This stage shows the signals worth buying and the decision they exist to trigger, not a monitoring product.
        </p>
      </div>
    </div>
  );
