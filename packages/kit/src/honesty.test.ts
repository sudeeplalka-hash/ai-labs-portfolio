...
describe("assertUseCases — honesty enforced at load", () => {
  ...
  it("throws on a first-hand claim without owner sign-off", () => {
    const unsigned = { kind: "first-hand", ownerSignedOff: false } as unknown as UseCase["provenance"];
    expect(() => assertUseCases([validUc({ provenance: unsigned })])).toThrow(/first-hand/i);
  });
  it("passes the signed first-hand constant", () => {
    expect(() => assertUseCases([validUc({ provenance: firstHand })])).not.toThrow();
  });
});

describe("coverageFrom — computed, never asserted", () => {
  it("counts totals and first-hand per industry and rolls up the summary", () => {
  ...
  it("reports zero first-hand domains when everything is studied", () => {
